...
export async function addComments(main) {
    ...
    `
    let commentTameplate=(author,content)=> html `
    <div id="comments">
    ...
    for(let i=0;i<parentContainers.length;i++) {
        ...
        let containerCommnets=parentContainers[i].getElementsByClassName('container-for-commnets')[0];
        let dataComments=await getDataComments(`${key}`);
        console.log('test')
        for(let x=0;x<dataComments.length;x++) {
            if(dataComments[x].commentId==parentContainers[i].id) {
                containerCommnets.appendChild(commentTameplate('author',dataComments[x].content));
            }
        }
    }
}